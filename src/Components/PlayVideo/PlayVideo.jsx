...
const PlayVideo = () => {
  const {videoId}=useParams();
     ...
     const fetchvideodata= async()=>{
       ...
       await fetch(videodetails_url).then(resp=>resp.json()).then(data=>setapidata(data.items[0]));
    }
  ...
  return (
    <div className='play-video'>
        ...
        <div className='play-video-info'>
            <p>{apidata?value_converter(apidata.statistics.viewCount):"16k"} views &bull; {apidata?apidata.snippet.publishedAt:"2 months"}</p>
            <div >
        ...
        <div className='publisher'>
            ...
            <div >
                <p>{apidata?apidata.snippet.channelTitle:"basic"}</p>
                <span>{channeldata?value_converter(channeldata.statistics.subscriberCount):"1M"}subscriber</span>
            </div>
            <button>subscribe</button>
        </div>
      <div className="vid-description">
       <p>{apidata?apidata.snippet.description.slice(0,500):"Description"}</p>
        <hr />
        <h4>{apidata?value_converter(apidata.statistics.commentCount):1093} comments</h4>
